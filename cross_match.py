...
def main(xref="", yref="", xobj="", yobj="", error=0.01, scale="", angle="",
    ...
    the scale. It is the scale by which you need to multiply xobj and yobj 
    so that they have the same as the reference coordinate system
    ...
    #else :
    #    coords_obj = zip(triangle_obj[0,:], 
    #                     triangle_obj[1,:], 
    #                     triangle_obj[2,:] * scale)
    #    coords_ref = zip(triangle_ref[0,:], 
    #                     triangle_ref[1,:], 
    #                     triangle_ref[2,:])
    ...
    scales_highest = distances_highest_ref[non_zero] / distances_highest_obj[non_zero]
    scale_estimate = np.median(scales_highest)
    scale_MAD_estimate = np.median(abs(scales_highest - scale_estimate))
    
 
    #print "scale_estimate, scale_MAD_estimate", scale_estimate, scale_MAD_estimate

    # If the numbers provided are exactly the same, the scale_MAD_estimate=0
    ...
    if scale_MAD_estimate/scale_estimate < 1E-8: # scale_MAD_estimate too small!
        scale_MAD_estimate = scale_estimate * 1E-8
    ...
    if angle != "":
        angle, angle_std, flip = angle, None, flip
    else:
